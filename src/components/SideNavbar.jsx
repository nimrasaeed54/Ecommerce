...
// }
import React, { useState } from "react";
import { FiMenu, FiHome, FiPackage, FiShoppingCart, FiUser } from "react-icons/fi";
import { Link } from "react-router-dom";
import Searchbar from "./Searchbar";
import logo from "../assets/navlogo.png";
...
export default function NavbarWithSidebar({ children }) {
  ...
  return (
    <div className="flex h-screen">
      ...
      <div
        className={`fixed top-0 left-0 h-full bg-slate-300 text-black transition-all duration-300 
        ${isOpen ? "w-40" : "w-0"} lg:w-32 flex flex-col items-center z-50 overflow-hidden`}
      >
        <ul className="mt-32 space-y-6 w-full">
          {[
            { name: "Home", icon: <FiHome size={20} />, link: "/" },
      ...
      <div className="flex-1 flex flex-col transition-all duration-300 lg:ml-40">
        ...
        <nav className="hidden lg:flex fixed top-0 left-0 w-full bg-slate-300 h-16 px-6 items-center justify-between z-50">
        ...
        <nav className="flex lg:hidden fixed top-0 left-0 w-full bg-slate-300 h-16 px-4 items-center justify-between z-50">
          <button className="p-2 text-black" onClick={() => setIsOpen(!isOpen)}>
            <FiMenu size={24} />
          </button>
        ...
        <div className="fixed top-16 left-0 w-full bg-white flex justify-center lg:hidden">
          <Searchbar onSearch={(query) => console.log(query)} />
        </div>
