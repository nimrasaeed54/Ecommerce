        ...
        {hotSellingShoes.map((shoe) => (
          <li key={shoe.id} className="flex items-center gap-4 border-b pb-3 last:border-none">
            <div className="relative">
              ...
              <span className="absolute -top-0 -right-2 bg-orange-500/90 shadow-md text-white text-[10px] px-2 py-1 rounded-full">
                HOT
              </span>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-gray-700">{shoe.name}</h3>
              <span className="text-red-600 font-bold text-sm">${shoe.price}</span>
            </div>
          </li>
        ))}
      </ul>
